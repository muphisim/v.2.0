    ...
    for sol in solvers:
        ...
        if keyFSI in sol.keys():
            ...
            for location in allLocations:
                nodeTags = model.getGroupOfNodes(nodeGroupIndex =(location[0],location[1]))
                eleTags = model.getGroupOfFacets(facetGroupIndex =(location[0],location[1]))
                for inod in nodeTags:
                    FSINodes.add(inod)
                for prEl in eleTags:
                    loc = prEl[0]
                    support = prEl[1]
                    for ele in support:
                        FSIElements.add((ele,loc))
            if len(FSINodes) >0:
                oxFile.write("*FSI\n")
                for inod in FSINodes:
                    oxFile.write("%d\n"%inod)
            
            if len(FSIElements) >0:
                oxFile.write("*ELEMENT FSI\n")
                for inod in FSIElements:
                    oxFile.write("%d,%d\n"%(inod[0],inod[1]))
        ...
        if key.upper()=="GRAVITY":
           oxFile.write("*GRAVITY\n%.16g\n"%(vals)) 
            ...
            for supp in vals:
                ...
                #print(values)
                parentTags = []
                if location != "ALL" and key.upper() not in ["ELEMENTPRESSUREBC", "ELEMENTSTRESSBC"]:
                    if location[0] ==  dim-1:
                ...
                if key.upper()=="PRESSUREBC":
                    ...
                    oxFile.write("%.16g,%.16g,%.16g,%d\n"%(t0,tf,values,loc))
                    oxFile.write("%d\n"%support)
                elif key.upper()=="ELEMENTSTRESSBC":
                    loc = location[1]
                    support = location[0]
                    oxFile.write("*BOUNDARY, TYPE=%s\n"%supp[0].upper())
                    oxFile.write("%.16g,%.16g,%.16g,%.16g,%.16g,%.16g,%.16g,%.16g,%d\n"%(t0,tf,values[0],values[1],values[2],values[3],values[4],values[5],loc))
                    oxFile.write("%d\n"%support)
                elif key.upper()=="STRESSBC":                        
                    for prEl in parentTags:
                        loc = prEl[0]
                        support = prEl[1]
                        oxFile.write("*BOUNDARY, TYPE=%s\n"%supp[0].upper())
