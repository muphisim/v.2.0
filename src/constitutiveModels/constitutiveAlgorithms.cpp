...
/*! \brief constructor
  @param[in] fieldIndexes indexes of the extraDof fields that the coupling occurs
*/
MechanicalExtraDofFullCouplingTermPoroElastic::MechanicalExtraDofFullCouplingTermPoroElastic(const vector<int>& fieldIndexes, double alpha): 
            MechanicalExtraDofFullCouplingTerm(fieldIndexes), _alpha(alpha){}
...
/*! \brief This function to get the stiffness matrix
  ...
  @param[out] stiff stiffness matrix
*/
void MechanicalExtraDofFullCouplingTermPoroElastic::getStiffnessMatrix(const classGPs *GaussPoint, mMatrix& stiff) const
{
    // take identical part from existing term
    MechanicalExtraDofFullCouplingTerm::getStiffnessMatrix(GaussPoint,stiff);
    // add stiffness related to fluid pressure
    ...
    for (int kk =0; kk< nbFields; kk++)
    {
        for (int a = 0; a < numNeighbours; a++) 
        {
            for (int i = 0; i < ndim; i++) 
            {
                ...
                for (int b=0; b< numNeighbours; b++)
                {
                    int indi2 = b + numNeighbours * (numberMechDofPerNode+_fieldIndexes[kk]);
                    stiff(indi11,indi2) += wJ * _alpha*dphi[b * ndim + i]* phi[a];
                }
            }
        }
    }
...
void MechanicalExtraDofFullCouplingTermPoroElastic::getForceVector(const classGPs *GaussPoint, mVector& vForce) const
{
    // take identical part from existing term
    MechanicalExtraDofFullCouplingTerm::getForceVector(GaussPoint, vForce);
    // add force contribution from fluid pressure
    const classStates& currState = GaussPoint->getCurrentState();
    const classStates& prevState = GaussPoint->getPreviousState(); 
    const vector<int>& neighbours = GaussPoint->getNeighbours();
    int numNeighbours = neighbours.size();
    ...
    for (int kk =0; kk< nbFields; kk++)
    {
        const classTensor1& gradp = currState.getExtraFieldGradients()[_fieldIndexes[kk]];
        for (int a = 0; a < numNeighbours; a++) 
        {
            for (int i = 0; i < ndim; i++) 
            {
                int indi11 = a + numNeighbours * i;
                vForce(indi11) += wJ * _alpha*gradp(i) * phi[a];
            }
        }
    }
}
