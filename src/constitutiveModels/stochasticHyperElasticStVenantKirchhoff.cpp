...
void classStochasticHyperElasticStVenantKirchhoff::stress(vector<double> &PK2, const vector<double> &E) {
    ...
    if (ndim == 2 && stressState != 1) {
        if (stressState == 0)//plane stress
        {
            ...
            *nstoch, 0);
            multiSTensorRandomVariableStochastic(PK2, ndim, ndim, _fac, nstoch, C_Stoch,
                    ...
                    for (int v = 0; v < nstoch; v++) {
                        for (int i = 0; i < ndim; i++) {
                            PK2[m * ndim * ndim + i * ndim + i] +=
                                    ...
                                    _nutimesYoung_over_one_minusnu_carre[v];
                        }
                    }
                }
            }
    ...
    } else//plane strain in 2D by definition or isotropic in 3d
    {
        vector<double> PK2_temp(ndim *ndim
        *nstoch, 0);
        multiSTensorRandomVariableStochastic(PK2, ndim, ndim, _mu2, nstoch, C_Stoch, PK2_temp);
        PK2 = PK2_temp;
        for (int m = 0; m < nstoch; m++) {
            for (int i = 0; i < ndim; i++) {
                for (int u = 0; u < nstoch; u++) {
                    for (int v = 0; v < nstoch; v++) {
                        PK2[m * ndim * ndim + i * ndim + i] += C_Stoch[m * nstoch * nstoch + u * nstoch + v] * _lam[u] *
...
void classStochasticHyperElasticStVenantKirchhoff::fillElasticStiffTensor(int ndim, int number_stochastic) {
    ...
    this->_nutimesYoung_over_one_minusnu_carre = nutimesYoung_over_one_minusnu_carre;                                               
...
*/
void classStochasticHyperElasticStVenantKirchhoff::build_CF(vector<double> &CF, const vector<double> &FCurr) {
    int ndim = getDim();
...
/*! \brief This function defines the product of C and F summed on one stochastic variable
  ...
  @param[in] FCurr Deformation tensor
*/
void classStochasticHyperElasticStVenantKirchhoff::build_dEdF(vector<double> &dEdF, const vector<double> &FCurr) {
    int ndim = getDim();
...
void classStochasticHyperElasticStVenantKirchhoff::updateConstitutive(classGPs *GaussPoint, double dt, double timeRun, bool flagTanMod)
{
    ...
    build_CF(CF, FCurr);
    build_dEdF(dEdF, FCurrMecha);
    if(_stochasticMapping){
        pushBackdEdF(dEdF,FIniInv);
    }

    build_dSdF(dSdF, dEdF);
    if(_stochasticMapping){
        pushBackdSdF(dSdF,FIniInv,FIniInvT,det_FIni);
    }
    if (flagTanMod) {
...
/*! \brief This function updates the constitutive response. This function is mandatory and it must be implemented by the user.
  ...
  @param[in] flagTanMod Flag to calculate the tangent modulus
*/
void classStochasticHyperElasticStVenantKirchhoff::getTanModuli(classTensor6* tanModuli, const vector<double> &FCurr, const vector<double> &PK2, const bool flagTanMod)
{
    int ndim = getDim();
    vector<double> CF(nstoch*nstoch*ndim*ndim,0);
    vector<double> dEdF(nstoch*nstoch*ndim*ndim*ndim*ndim,0);
...
/*! \brief This function updates the constitutive response. This function is mandatory and it must be implemented by the user.
  ...
  @param[in] flagTanMod Flag to calculate the tangent modulus
*/
void classStochasticHyperElasticStVenantKirchhoff::pushBackdEdF(vector<double> &dEdF, const vector<double> &FIniInv)
{
    int ndim = getDim();
    vector<double> dFdF(nstoch*nstoch*ndim*ndim*ndim*ndim,0);
    vector<double> dFedF(nstoch*nstoch*ndim*ndim*ndim*ndim,0);   
    const vector<double> dedf = dEdF;
    setAll(dEdF,0);

    for(int alpha =0; alpha<nstoch; alpha++){
        for(int i =0; i<ndim; i++){
            for(int j =0; j<ndim; j++){
                dFdF[alpha*ndim*ndim*ndim*ndim*nstoch + alpha*ndim*ndim*ndim*ndim + i*ndim*ndim*ndim + j*ndim*ndim + i*ndim + j] = 1;
            }
        }
    }

    derivativeProductTensorConstantTensorRandomVariable(dFdF, FIniInv, "RIGHT", dFedF, C_Stoch, ndim, nstoch);
                    ...
                    for (int k = 0; k < ndim; k++) {
                        for (int M = 0; M < ndim; M++) {
                            for (int psi = 0; psi < nstoch; psi++) {
                                for (int R = 0; R < ndim; R++) {
                                    for (int S = 0; S < ndim; S++) {
                                           dEdF [beta*ndim*ndim*ndim*ndim*nstoch+ i*ndim*ndim*ndim*nstoch + J*ndim*ndim*nstoch+ eps*ndim*ndim +k*ndim + M] +=
                                                   dedf[beta*ndim*ndim*ndim*ndim*nstoch+ i*ndim*ndim*ndim*nstoch + J*ndim*ndim*nstoch+ psi*ndim*ndim + R*ndim + S]*dFedF[psi*ndim*ndim*ndim*ndim*nstoch + eps*ndim*ndim*ndim*ndim + R*ndim*ndim*ndim + S*ndim*ndim +  k*ndim + M];
                                    }
                                }
                            }
                        }
...
/*! \brief This function updates the constitutive response. This function is mandatory and it must be implemented by the user.
  ...
  @param[in] flagTanMod Flag to calculate the tangent modulus
*/
void classStochasticHyperElasticStVenantKirchhoff::pushBackdSdF(vector<double> &dSdF, const vector<double> &FIniInv, const vector<double> &FIniInvT, const vector<double> &det_FIni)
{
    int ndim = getDim();
    vector<double> Cdet_FInidF (nstoch*nstoch,0);
    vector<double> ddet_FInidF (nstoch*nstoch*ndim*ndim,0);
    vector<double> dSdF_FiniinvT(nstoch*nstoch*ndim*ndim*ndim*ndim,0);
    vector<double> Finiinv_dSdF_FiniinvT(nstoch*nstoch*ndim*ndim*ndim*ndim,0);
    vector<double> dsdf = dSdF;
                    ...
                    for(int d =0; d < ndim; d++){
                        for(int a =0; a < ndim; a++){
                            for(int b =0; b < ndim; b++){
                                dSdF[alpha*ndim*ndim*ndim*ndim*nstoch + beta*ndim*ndim*ndim*ndim + c*ndim*ndim*ndim + d*ndim*ndim + a*ndim +b] = dsdf[alpha*ndim*ndim*ndim*ndim*nstoch + c*ndim*ndim*ndim*nstoch + d*ndim*ndim*nstoch + beta*ndim*ndim + a*ndim +b];
                            }
                        }
                    }
                }
    ...
    derivativeProductTensorConstantTensorRandomVariable(dSdF, FIniInvT, "RIGHT", dSdF_FiniinvT, C_Stoch, ndim, nstoch);
    derivativeProductTensorConstantTensorRandomVariable(FIniInv, dSdF_FiniinvT, "LEFT", Finiinv_dSdF_FiniinvT, C_Stoch, ndim, nstoch);
    setAll(dSdF,0);

    for(int alpha =0; alpha<nstoch; alpha++){
        for(int beta =0; beta < nstoch; beta++){
            for(int gamma =0; gamma < nstoch; gamma++){
                Cdet_FInidF[gamma*nstoch + beta] += C_Stoch[gamma*nstoch*nstoch + beta*nstoch + alpha]*det_FIni[alpha];
            }
        }
    }
            ...
            for(int gamma =0; gamma < nstoch; gamma++){
                for(int c =0; c < ndim; c++){
                    for(int d =0; d < ndim; d++){
                        for(int a =0; a < ndim; a++){
                            for(int b =0; b < ndim; b++){
                             dSdF[gamma*nstoch*ndim*ndim*ndim*ndim + a*ndim*ndim*ndim*nstoch + b*ndim*ndim*nstoch + alpha*ndim*ndim + c*ndim +d] += Cdet_FInidF[gamma*nstoch + beta]*Finiinv_dSdF_FiniinvT[beta*ndim*ndim*ndim*ndim*nstoch + alpha*ndim*ndim*ndim*ndim + a*ndim*ndim*ndim + b *ndim*ndim + c*ndim +d];
                            }
                        }
                    }
                }
