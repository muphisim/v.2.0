...
classCompactMatrix::classCompactMatrix(int r, int c, string type) : classMatrix(r, c, type), _matAllocated(false) {
...
void classCompactMatrix::luSolve(const vector<double>& rhs, vector<double>&res)
{
    if ( _r != _c) 
    {
        ERROR("The matrix must be square to call luSolve");
        exit(-1);
    }
    int N = _r, nrhs = 1, lda = N, ldb = N, info;
    int ipiv[N];
    res = rhs;
    ...
    for (int i=0; i< _r; i++)
    {
        for (int j=0; j<_c; j++)
        {
          data[i+_r*j]= _data[i * _c + j];
        }
    }
    
    dgesv_(&N, &nrhs, data, &lda, ipiv, &res[0], &ldb, &info);

    if(info != 0)
    {
        ERROR("The solution could not be computed");
        exit(-1);
    }
}
...
void mMatrix::invert(mMatrix& inv) const
{
    inv.resize(nrows, ncols, true);
    if (nrows == ncols)
    {
        inverse(values, nrows, inv.values);
    }
    else
    {
...
double mMatrix::det() const
{
    if (nrows == ncols)
    {
        return determinantTensor(values, nrows);
    }
    else
    {
        ERROR("matrix is not square");
...
/*! \brief This function calculates the stochastic trace of a tensor
  @param[in] A Stochastic tensor to calculate its trace
  @param[in] ndim Physical Dimension of the tensor
  @param[in] nstoch Stochastic Dimension of the tensor
  @param[in&out] Trace Stochastic trace of the tensor
 */

extern void getStochasticTrace(const vector<double> &A, const int ndim, const int nstoch, vector<double> &Trace) {
    setAll(Trace,0);

    for (int alpha =0; alpha < nstoch; alpha ++){
        for (int i = 0; i < ndim; i++) {
            Trace[alpha] += A[alpha*ndim*ndim + i * ndim + i];
        }
    }

}

/*! \brief This function calculates the deviatoric part of a stochastic tensor
  @param[in] A Stochastic tensor to calculate its trace
  @param[in] ndim Physical Dimension of the tensor
  @param[in] nstoch Stochastic Dimension of the tensor
  @param[in&out] devA Stochastic deviatoric part of the tensor
 */

extern void getStochasticDev(const vector<double> &A, const int ndim, const int nstoch, vector<double> &devA) {
    setAll(devA,0);
    vector<double> Trace(nstoch,0);
    getStochasticTrace(A,ndim,nstoch,Trace);
    devA  = A;

    for (int alpha =0; alpha < nstoch; alpha ++){
        for (int i = 0; i < ndim; i++) {
            devA[alpha*ndim*ndim + i*ndim + i] -= (1/double(ndim))*Trace[alpha];
        }
    }
...
/*! \brief Stochastic Dyadic product between two vectors
  @param[in] A First random vector
  @param[in] ndim Physical dimension
  @param[in] B Second random vector
  @param[in] nstoch Stochastic dimension
  @param[in] C_stoch Size of the second vector
  @param[in] C Result (AXB)
 */

extern void stochasticDyadicProduct(const vector<double> &A, int ndim, const vector<double> B, int nstoch, const vector<double> &C_Stoch, vector<double> &C){
    ...
    for(int alpha =0; alpha < nstoch; alpha++){
        for(int beta =0; beta < nstoch; beta++){
            for(int gamma =0; gamma < nstoch; gamma++){
                for(int i =0; i < ndim; i++){
                    for(int j =0; j < ndim; j++){
                        C[gamma*ndim*ndim + i*ndim + j] += C_Stoch[alpha + beta*nstoch + gamma*nstoch*nstoch]*A[alpha*ndim + i]*B[beta*ndim + j];
                    }
                }
            }
        }
...
/*! \brief Compute derivatives of the product of a constant RV and a variable RV.
  @param[in&out] dJdF
  @param[in&out] dFdF
  ...
  @param[in] nstoch
 */
extern void derivativeProductRandomConstantRandomVariable(const vector<double> &A,const vector<double> &dBdX, vector<double> &AdBdX,const vector<double> &C_Stoch,const int ndim,const int nstoch) {
    setAll(AdBdX,0);
    vector<double> CA(nstoch*nstoch,0);

    for(int alpha =0; alpha < nstoch; alpha++){
        for(int beta =0; beta < nstoch; beta++){
            for(int gamma =0; gamma < nstoch; gamma++){
                CA[gamma*nstoch + beta ] += C_Stoch[gamma*nstoch*nstoch + beta*nstoch + alpha]*A[alpha];
            }
        }
    }
    for(int alpha =0; alpha < nstoch; alpha++){
        for(int beta =0; beta < nstoch; beta++){
            for(int gamma =0; gamma < nstoch; gamma++){
                for(int a =0; a < ndim; a++){
                    ...
                    AdBdX [gamma *ndim*ndim*nstoch + alpha*ndim*ndim + a*ndim +b] += CA[gamma*nstoch + beta]*dBdX[beta*nstoch*ndim*ndim + alpha*ndim*ndim + a*ndim +b];
                    }
                }
            }
        }
...
extern void derivativeProductTensorConstantTensorRandomVariable(const vector<double> &A, const vector<double> B, const string order, vector<double> &Result, const vector<double> &C_Stoch,const int ndim,const int nstoch){

    setAll(Result,0);
    vector<double> CA(nstoch*nstoch*ndim*ndim,0);
    vector<double> CB(nstoch*nstoch*ndim*ndim,0);
    if (order.compare(0, 4, "LEFT") == 0){
        //The third order tensor is on the left.
        for(int beta=0; beta<nstoch; beta++){
            for(int gamma=0; gamma<nstoch; gamma++){
                for(int alpha=0; alpha<nstoch; alpha++){
                    for(int i=0; i<ndim; i++){
                        for(int m=0; m<ndim; m++){
                            CA[gamma*nstoch*ndim*ndim + beta*ndim*ndim + i*ndim + m] += C_Stoch[gamma*nstoch*nstoch + beta*nstoch + alpha]*A[alpha*ndim*ndim + i*ndim + m];
                        }
                    }
                }
            }
                    ...
                    for(int i=0; i<ndim; i++){
                        for(int j=0; j<ndim; j++){
                            for(int k=0; k<ndim; k++){
                                for(int l=0; l<ndim; l++){
                                   for(int m=0; m<ndim; m++){     
                                        Result[gamma*nstoch*ndim*ndim*ndim*ndim + delta*ndim*ndim*ndim*ndim + i*ndim*ndim*ndim + j*ndim*ndim + k*ndim + l] += CA[gamma*nstoch*ndim*ndim + beta*ndim*ndim + i*ndim + m] * B[beta*nstoch*ndim*ndim*ndim*ndim + delta*ndim*ndim*ndim*ndim + m*ndim*ndim*ndim + j*ndim*ndim + k*ndim +l];
                                   }
                                }
                            }
                        }
    ...
    } else if (order.compare(0, 5, "RIGHT") == 0) {
        //The third order tensor is on the right.
        for(int beta=0; beta<nstoch; beta++){
            for(int gamma=0; gamma<nstoch; gamma++){
                for(int alpha=0; alpha<nstoch; alpha++){
                    for(int m=0; m<ndim; m++){
                        for(int j=0; j<ndim; j++){
                            CB[gamma*nstoch*ndim*ndim + alpha*ndim*ndim + m*ndim + j] += C_Stoch[gamma*nstoch*nstoch + beta*nstoch + alpha]*B[beta*ndim*ndim + m*ndim + j];
                        }
                    }
                }
            }
                    ...
                    for(int i=0; i<ndim; i++){
                        for(int j=0; j<ndim; j++){
                            for(int k=0; k<ndim; k++){
                                for(int l=0; l<ndim; l++){
                                   for(int m=0; m<ndim; m++){     
                                        Result[gamma*nstoch*ndim*ndim*ndim*ndim + delta*ndim*ndim*ndim*ndim + i*ndim*ndim*ndim + j*ndim*ndim + k*ndim + l] +=  A[alpha*nstoch*ndim*ndim*ndim*ndim + delta*ndim*ndim*ndim*ndim + i*ndim*ndim*ndim + m*ndim*ndim + k*ndim +l]*CB[gamma*nstoch*ndim*ndim + alpha*ndim*ndim + m*ndim + j];
                                   }
                                }
                            }
                        }
...
/*! \brief Compute derivatives.
  ...
  @param[in] nstoch
 */
extern void derivativeProductRandomVariableRandomVariable(const vector<double> &A,const vector<double> &B, const vector<double> &dAdX,const vector<double> &dBdX, vector<double> &dABdX,const vector<double> &C_Stoch,const int ndim,const int nstoch) {
    setAll(dABdX,0);
    vector<double> CA(nstoch*nstoch,0);
    vector<double> CB(nstoch*nstoch,0);

    for(int alpha =0; alpha < nstoch; alpha++){
        for(int beta =0; beta < nstoch; beta++){
            for(int gamma =0; gamma < nstoch; gamma++){
                CA[gamma*nstoch + beta ] += C_Stoch[gamma*nstoch*nstoch + beta*nstoch + alpha]*A[alpha];
                CB[gamma*nstoch + beta ] += C_Stoch[gamma*nstoch*nstoch + beta*nstoch + alpha]*B[alpha];
            }
        }
    }
    for(int alpha =0; alpha < nstoch; alpha++){
        for(int beta =0; beta < nstoch; beta++){
            for(int gamma =0; gamma < nstoch; gamma++){
                for(int a =0; a < ndim; a++){
                    for(int b =0; b < ndim; b++){
                    dABdX [gamma *ndim*ndim*nstoch + alpha*ndim*ndim + a*ndim +b] += CA[gamma*nstoch + beta]*dBdX[beta*nstoch*ndim*ndim + alpha*ndim*ndim + a*ndim +b] + CB[gamma*nstoch + beta]*dAdX[beta*nstoch*ndim*ndim + alpha*ndim*ndim + a*ndim +b];
                    }
                }
            }
        }
...
/*! \brief Compute derivatives.
  ...
  @param[in] nstoch
 */
extern void derivativeProductRandomVariableTensorRandomVariable(const vector<double> &A,const vector<double> &TensorB, const vector<double> &dAdX,const vector<double> &dTensorBdX, vector<double> &dATensorBdX,const vector<double> &C_Stoch,const int ndim,const int nstoch) {
    setAll(dATensorBdX,0);
    vector<double> CA(nstoch*nstoch,0);
    vector<double> CTensorB(ndim*ndim*nstoch*nstoch,0);
    ...
    for(int alpha =0; alpha < nstoch; alpha++){
        for(int beta =0; beta < nstoch; beta++){
            for(int gamma =0; gamma < nstoch; gamma++){
                for(int a =0; a<ndim; a++){
                    for(int b =0; b<ndim; b++){
                    CTensorB[gamma*ndim*ndim*nstoch + beta*ndim*ndim + a*ndim + b] += C_Stoch[gamma*nstoch*nstoch + beta*nstoch + alpha]*TensorB[alpha*ndim*ndim + a*ndim + b];
                    }
                }
            }
        }
                    ...
                    for(int b =0; b < ndim; b++){
                        for(int c =0; c < ndim; c++){
                            for(int d =0; d < ndim; d++){
                                dATensorBdX [gamma*ndim*ndim*ndim*ndim*nstoch + alpha*ndim*ndim*ndim*ndim + a*ndim*ndim*ndim +b*ndim*ndim + c*ndim +d] += CA[gamma*nstoch + beta]*dTensorBdX[beta*nstoch*ndim*ndim*ndim*ndim + alpha*ndim*ndim*ndim*ndim + a*ndim*ndim*ndim + b*ndim*ndim + c*ndim +d] + CTensorB[gamma*ndim*ndim*nstoch + beta*ndim*ndim + a*ndim + b]*dAdX[beta*nstoch*ndim*ndim + alpha*ndim*ndim + c*ndim +d];
                            }
                        }
                    }
                }
...
/*! \brief Compute derivatives.
  ...
  @param[in] nstoch
 */
extern void derivativeProductRandomConstantTensorRandomVariable(const vector<double> &A, const vector<double> &dTensorBdX, vector<double> &dATensorBdX,const vector<double> &C_Stoch,const int ndim,const int nstoch){
    setAll(dATensorBdX,0);
    vector<double> CA(nstoch*nstoch,0);
            ...
            for(int gamma =0; gamma < nstoch; gamma++){
                for(int a =0; a < ndim; a++){
                    for(int b =0; b < ndim; b++){
                        for(int c =0; c < ndim; c++){
                            for(int d =0; d < ndim; d++){
                                dATensorBdX [gamma*ndim*ndim*ndim*ndim*nstoch + alpha*ndim*ndim*ndim*ndim + a*ndim*ndim*ndim +b*ndim*ndim + c*ndim +d] += CA[gamma*nstoch + beta]*dTensorBdX[beta*nstoch*ndim*ndim*ndim*ndim + alpha*ndim*ndim*ndim*ndim + a*ndim*ndim*ndim + b*ndim*ndim + c*ndim +d];
                            }
                        }
                    }
                }
...
extern void InverseSystem(const vector<double> &A, const vector<double> &B, const int ndim, const int nstoch,
                                                           vector<double> &C) {
// Compute Ax = B and store the result in DC

// build the matrix sum_i BiCijk
    setAll(C,0);
    classMatrix *KKp;
    KKp = new classCompactMatrix(ndim*ndim*nstoch, ndim*ndim*nstoch, "COMPACT");
    double val = 0;
    int row = 0;
    int col = 0;
    for (int alpha = 0; alpha < nstoch; alpha++) {
        for (int i = 0; i < ndim; i++) {
            for (int j = 0; j < ndim; j++) {
                for (int beta = 0; beta < nstoch; beta++) {
                    for (int k = 0; k < ndim; k++) {
                        for (int l = 0; l < ndim; l++) {
                        val = A[beta*ndim*ndim*ndim*ndim*nstoch + alpha*ndim*ndim*ndim*ndim + k*ndim*ndim*ndim + l*ndim*ndim + i*ndim +j];
                        row = beta*ndim*ndim + k*ndim + l;
                        col = alpha*ndim*ndim + i*ndim +j;
                        KKp->addIJ(row, col, val);
                        val = 0;
                        }
                    }
                }
