...
/*! \brief remove space at the leading of string
*/
...
/*! \brief remove space at the trailing of string
*/
...
/*! \brief remove space at the leading and trailing of string
*/
...
extern void elementsManagement(vector<classElements *> &elements, int ndim, ifstream &ifs, vector<classNodes *> &nodes,
                               vector<int> &epart, vector<int> &nodLocal) {
...
extern void materialManagement(vector<classElements *> &elements, ifstream &ifs, int ndim,
    ...
    while (getline(ifs, line)) {
        if (line.compare(0, 9, "*MATERIAL") == 0) {
            ...
            if (line.compare(0, 8, "*Density") == 0 && flagNode) {
			...
			} else if (line.compare(0, 19, "*Linear-Poroelastic") == 0 && flagNode) {
                getline(ifs, templine);
                tmp = int(templine.size());
                int contc = 0;
                commas.clear();
                ...
                name_cons = "Linear-Poroelastic";
                constitutiveModel = new LinearPoroelastic(ndim, name, name_cons, rho, young, nu, alpha, stressState);
                commas.clear();
                consMod.push_back(constitutiveModel);
		...
		constitutiveModel = new class3DprintingThermoelasticityStVenantKirchhoff(ndim, name, name_cons, rho,
                ...
                if (templine.compare(0, 8, "*FHNelec") == 0) {
                    ...
                    consMod.push_back(constitutiveModel);
				} else if (templine.compare(0, 9, "*DarcyLaw") == 0) {
					name_cons="DarcyLaw";
                    getline(ifs, templine);
                    tmp = int(templine.size());
                    commas.clear();
                    ...
                    commas.push_back(tmp + 1);
                    templine.copy(tmpme2, commas[0], 0);
                    double kappa = atof(tmpme2);
                    memset(tmpme2, 0, maxsize);
					//
                    templine.copy(tempNode, commas[1] - commas[0] - 1, commas[0] + 1);
                    double mu = atof(tempNode);
                    memset(tempNode, 0, maxsize);
					//
                    templine.copy(tempNode, commas[2] - commas[1] - 1, commas[1] + 1);
                    double Q = atof(tempNode);
                    memset(tempNode, 0, maxsize);
					//
                    templine.copy(tempNode, commas[3] - commas[2] - 1, commas[2] + 1);
                    double alpha = atof(tempNode);
                    ...
                    commas.clear();
                    constitutiveModel = new DarcyLaw(ndim, name, name_cons, kappa , mu, Q, alpha);
                    consMod.push_back(constitutiveModel);
                ...
                if ( flagNode) {
                    if(templine.compare(0, 13, "*HyperElastic") == 0){
                        ...
                        if ((!templine.compare(0, 16, "*IsoMorphoGrowth") == 0) &&
                        (!templine.compare(0, 11, "*AreaGrowth") == 0) && (!templine.compare(0, 16, "*ViscoPlasticity") == 0)){
                        ERROR("The %s model is not implemented in MuPhiSim", line.c_str());
                        exit(-1);    
                        }
                    }
                    ...
                    double n = 0;
                    int stressState = 0;
                    if((stochModel.compare(0, 16, "*IsoMorphoGrowth") == 0) || (stochModel.compare(0, 11, "*AreaGrowth") == 0) ){
                    commas.push_back(tmp + 1);
                    templine.copy(tmpme2, commas[0], 0);
                    young = atof(tmpme2);
                    memset(tmpme2, 0, maxsize);
                    ...
                    if(stochModel.compare(0, 16, "*ViscoPlasticity") == 0){
                        getline(ifs, templine);
                        commas.clear();
                        tmp = int(templine.size());
                        int contc = 0;
                        ...
                        n = atof(tempNode);
                        memset(tempNode, 0, maxsize);
                        INFO("The value of eps_0 = %g",eps_0);
                        INFO("The value of s = %g",s);
                        INFO("The value of n = %g",n);
                    }
                    commas.clear();
                    getline(ifs, templine);
                    int order;
                    ...
                    int cont_stochastic_function = 0;
                    ...
                    printf("%s\n",stochModel.c_str());
                    if (stochModel.compare(0, 11, "*AreaGrowth") == 0) {
                    name_cons = "AreaGrowth";
                    INFO("The stochastic constitutive model is AreaGrowth");
                    constitutiveModel = new classStochasticAreaGrowth (ndim, name, name_cons, rho,
                                                                                         young, nu, Gc,
                                                                                         stochastic_number,
                    ...
                    } else if (stochModel.compare(15, 34, "St-Venant-Kirchhoff") == 0) {
                    ...
                    constitutiveModel = new classStochasticHyperElastic3DCompressibleNeoHookean(ndim, name, name_cons, rho,
                                                                                         ...
                                                                                         approximation, order,
                                                                                         resolution, stochasticMapping, distribution);    
                    } else if (stochModel.compare(0, 16, "*IsoMorphoGrowth") == 0) {
                    name_cons = "IsoMorphoGrowth";
                    INFO("The stochastic constitutive model is IsoMorphoGrowth");
                    constitutiveModel = new classStochasticIsoMorphogeneticGrowth (ndim, name, name_cons, rho,
                                                                                         young, nu, Gc,
                                                                                         stochastic_number,
                                                                                         stochastic_parameters,
                                                                                         stochastic_function,
                                                                                         approximation, order,
                                                                                         resolution, stochasticMapping, distribution);    
                    } else if (stochModel.compare(0, 16, "*ViscoPlasticity") == 0) {
                    name_cons = "ViscoPlasticity";
                    INFO("The stochastic constitutive model is ViscoPlasticity");
                    constitutiveModel = new classStochasticViscoPlasticity (ndim, name, name_cons, rho,
                                                                                         young, nu, eps_0, s, n,
                                                                                         stressState,
                                                                                         stochastic_number,
                                                                                         stochastic_parameters,
            ...
            while (getline(ifs, line2)) {
                if (line2.compare(0, 12, "*END SOLVER") == 0) {
                    ...
                    if (line2.compare(0, 11, "*ACTIVATION") == 0) {
                        ...
                        flagFSI = true;
                        INFO("FSI simulation, nodes at FSI interface are set");
                    } else if (line2.compare(0, 12, "*ELEMENT FSI") == 0) {
                        flagFSI = true;
                        INFO("FSI simulation, element at FSI interface are set");
                        ...
                        while (getline(ifs, secondLine)) {
                            ...
                            else
                            {
                                ...
                                if (commas.size() == 1)
                                {
                                  ...
                                  memset(tmpme2, 0, maxsize);
                                  secondLine.copy(tmpme2, secondLine.size(), commas[1 - 1] + 1);
                                  int step = atoi(tmpme2);
                                  secondLine.clear();
                                  INFO("time = %e step =%d",tt, step);
                                ...
                                else if (commas.size() == 2)
                                {
                                  ...
                                  double tt = atof(tmpme2);
                                  memset(tmpme2, 0, maxsize);
                                  secondLine.copy(tmpme2, commas[1]-commas[0]-1 , commas[1 - 1] + 1);
                                  int step = atoi(tmpme2);
                                  
                                  memset(tmpme2, 0, maxsize);
                                  secondLine.copy(tmpme2, secondLine.size()-commas[1]-1, commas[2 - 1] + 1);
                                  double dtmax = atof(tmpme2);
                                  INFO("time = %e step =%d, dtMax=%e",tt, step, dtmax);
                                  secondLine.clear();
                                  steppingPlan.setNumberOfSteps(tt,step, dtmax);
                                }
                                else
                                {
                                  ERROR("format is wrong with %d commas in *TIME STEPPING PLAN",commas.size());
                                }
                                oldposLoc = ifs.tellg();
                            }
                        }
...
/*! \brief Load the force BCs from the input file
  ...
  @param[in] nodLocal Mapping of local nodes to global nodes numbering nodlocal[local node index]= global node index (empty for sequential simulations)
*/
extern void stressInstBCManagement(vector<classElements *> &elements, vector<classNodes *> &nodes,
                                     vector<classNeumannBCs *> &NeumannBCs, int ndim, ifstream &ifs, vector<int> &epart,
                                     vector<int> &elemLocal) {

    classNeumannBCs *nodForces;
    string type = "STRESS INST";
    vector<double> tempxyz;
    vector<int> commas;
    int maxsize = 100;
    char tmpme2[maxsize], tempU[maxsize];
    ...
    Szz = atof(tmpme2);
    memset(tmpme2, 0, maxsize);
    line.copy(tmpme2, line.size(), commas[8 - 1] + 1);
    S = atof(tmpme2);
    memset(tmpme2, 0, maxsize);
    line.clear();
    INFO("t0=%g, t1=%g, Value of S is %g,%g,%g,%g,%g,%g, S is %g",t0, tf, Sxx, Sxy, Sxz, Syy, Syz, Szz, S);
#ifdef PARALLEL
    vector<int>::iterator it;
    int rank;
    PetscErrorCode ierr;
    ...
    nodForces = new classStressInst(type, elemsPress, Sxx, Sxy, Sxz, Syy, Syz, Szz, t0, tf);
    NeumannBCs.push_back(nodForces);
}
...
/*! \brief Load the force BCs from the input file
  ...
  @param[in] nodLocal Mapping of local nodes to global nodes numbering nodlocal[local node index]= global node index (empty for sequential simulations)
*/
extern void stressRampBCManagement(vector<classElements *> &elements, vector<classNodes *> &nodes,
                                     vector<classNeumannBCs *> &NeumannBCs, int ndim, ifstream &ifs, vector<int> &epart,
                                     vector<int> &elemLocal) {

    classNeumannBCs *nodForces;
    string type = "STRESS RAMP";
    vector<double> tempxyz;
    vector<int> commas;
    int maxsize = 100;
    char tmpme2[maxsize], tempU[maxsize];
    ...
    nodForces = new classStressRamp(type, elemsPress, Sxx, Sxy, Sxz, Syy, Syz, Szz, t0, tf);
    NeumannBCs.push_back(nodForces);
}
    ...
    while (getline(ifs, line)) {
        if (line.compare(0, 9, "*BOUNDARY") == 0) {
            ...
            if (line.compare(16, tmp2, "FORCE") == 0) {
                ...
                tractionRampBCManagement(elements, nodes, NeumannBCs, ndim, ifs, epart, elemLocal);
            } else if (line.compare(16, 29, "STRESS RAMP") == 0) {
                stressRampBCManagement(elements, nodes, NeumannBCs, ndim, ifs, epart, elemLocal);
            } else if (line.compare(16, 29, "STRESS INST") == 0) {
                stressInstBCManagement(elements, nodes, NeumannBCs, ndim, ifs, epart, elemLocal);
            } else if (line.compare(16, 29, "PRESSURE RAMP") == 0) {
                pressureRampBCManagement(elements, nodes, NeumannBCs, ndim, ifs, epart, elemLocal);
            } else if (line.compare(16, 29, "PRESSURE INST") == 0) {
                pressureInstBCManagement(elements, nodes, NeumannBCs, ndim, ifs, epart, elemLocal);
            } else if (line.compare(16, 41, "STOCHASTIC PRESSURE RAMP") == 0) {
                pressureStochasticRampBCManagement(elements, nodes, NeumannBCs, ndim, ifs, epart, elemLocal);
            }
        }
        ...
        if (line.compare(0, 17, "*HERTZIAN CONTACT") == 0) {
            hertzianRampBCManagement(elements, nodes, NeumannBCs, ndim, ifs, epart, elemLocal);
        }
        if (line.compare(0, 28, "*STOCHASTIC HERTZIAN CONTACT") == 0) {
            stochasticHertzianRampBCManagement(elements, nodes, NeumannBCs, ndim, ifs, epart, elemLocal);
        }
        if (line.compare(0, 14, "*FLUX EXTRADOF") == 0) {
            if (line.compare(21, 33, "CURRENT INST") == 0) {
                ...
                heatInstBCManagement(elements, nodes, NeumannBCs, ndim, ifs, epart, elemLocal);
            } else if (line.compare(21, 31, "CONVECTION") == 0) {
                convectionBCManagement(elements, nodes, NeumannBCs, ndim, ifs, epart, elemLocal);
            } else if (line.compare(21, 30, "RADIATION") == 0) {
                radiationBCManagement(elements, nodes, NeumannBCs, ndim, ifs, epart, elemLocal);
        ...
        if (line.compare(0, 4, "*FSI") == 0) {
            FSIManagementNode(nodes, ifs, npart,nodLocal);
        }
        if (line.compare(0, 12, "*ELEMENT FSI") == 0) {
            FSIManagementElement(elements, nodes, ndim, ifs, epart, elemLocal);
        }
    }
}
...
/*! \brief Load the force BCs from the input file
  ...
  @param[in] ndim Dimension of the problem
  @param[in] ifs Pointer to the input file (.inp)
  @param[in] epart Processor ownership mapping 
  @param[in] elemLocal Mapping of local elements
*/
extern void FSIManagementElement(vector<classElements *> &elements, vector<classNodes *> &nodes,
                                  int ndim, ifstream &ifs, vector<int> &epart,
                                    vector<int> &elemLocal) {
#ifdef FSI
    vector<double> tempxyz;
    vector<int> commas;
    ...
    while (getline(ifs, line)) {
        if (line.compare(0, 1, "*") == 0) {
            ...
            if (epart.size() != 0) {
                ...
                int mainEle = atoi(tmpme2) - 1;
                memset(tmpme2, 0, maxsize);
                line.copy(tmpme2,size , commas[0]+1);
                int S = atoi(tmpme2);
                memset(tmpme2, 0, maxsize);
    ...
    FSIAdapter* adapter = FSIAdapter::getInstance();
    adapter->setBoundaryElements(elemsPress);
#endif
};
...
*/
extern void FSIManagementNode(vector<classNodes *> &nodes, ifstream &ifs, vector<int> npart, vector<int> &nodLocal) {
#ifdef FSI
    int maxsize = 100;
    string type = "FSI";
    ...
    FSIAdapter* adapter = FSIAdapter::getInstance();
    adapter->setBoundaryNodes(boundary_nodes);
    adapter->setBoundaryNodeCoordinates(boundary_coordinates);
#endif
};
