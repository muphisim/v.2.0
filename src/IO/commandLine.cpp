...
bool GeneralOptions::petscOptionsFromFileFlag=false;
...
void GeneralOptions::initialise(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) 
    {
        ...
        else if (strcmp(argv[i], "-FSIConfigFile")==0)
        {
          // start FSI instance
          std::string FSIConfigFile="";
          FSIConfigFile.append(argv[i+1]);
          INFO("FSI config file %s",FSIConfigFile.c_str());
#ifdef FSI
          FSIAdapter* adapter = FSIAdapter::getInstance();
          adapter->loadFSISettingFromFile(FSIConfigFile);
#endif //FSI
        }
    }
    ...
    {
        GeneralOptions::petscOptionsFromFileFlag = true;
        INFO("petsc options file %s exists, petsc options from this file are used !!!",GeneralOptions::petscOptionsFile.c_str());
...
extern void inputManagement(vector<classNodes *> &nod, vector<classElements *> &elem,
    ...
    if (ifs.is_open()) {
        ...
        ifs.clear();
        ifs.seekg(0);
        elementsManagement(elem, ndim, ifs, nod, epart, nodLocal);
        if (elem.size() == 0) {
