...
extern void NeumannBCManagement(const vector<double> &uK, vector<classGPs *> &GPs, vector<classNodes *> &nodes, vector<classElements *> &elements,
    ...
    for (int i = 0; i < NeumannBCs.size(); i++) {
        if (NeumannBCs[i]->getType() == "FORCE") {
        ...
        } else if (NeumannBCs[i]->getType() == "STRESS RAMP") {
            classStressRamp *nodas = static_cast<classStressRamp *>(NeumannBCs[i]);
            BCsStressRamp(uK, nodes, elements, nodas, ndim, force, timeRun, dt);
        } else if (NeumannBCs[i]->getType() == "STRESS INST") {
            classStressInst *nodas = static_cast<classStressInst *>(NeumannBCs[i]);
            BCsStressInst(uK, nodes, elements, nodas, ndim, force, timeRun, dt);
            ...
            BCsVolHeatFluxGaussian(uK,GPs, nodes, nodas, ndim, force, timeRun, dt);
        }
...
extern void BCsStressRamp(const vector<double> &uK, vector<classNodes *> &nodes, vector<classElements *> &elements,
    ...
    std::vector<double> stressTensor(9,0.);  // make a 9-len vector to represent the stress tensor
    BCsTraction->getTraction(timeRun, dt,stressTensor[0],stressTensor[1],stressTensor[2],stressTensor[4],stressTensor[5],stressTensor[8]);
    vector<double> stress(ndim * ndim, 0);
    if (ndim ==2){
        stress[0] = stressTensor[0];
        stress[1] = stressTensor[1];
        stress[2] = stressTensor[1];
        stress[3] = stressTensor[4];
        //INFO("in 2D, tensor has entries (%g,%g;%g,%g)",stress[0],stress[1],stress[2],stress[3]);
    }
    else if (ndim ==3){    //fill in symmetric off-diagonal entries
        stress = stressTensor;
        stress[3] = stressTensor[1];
        stress[6] = stressTensor[2];
        stress[7] = stressTensor[5];
    }
    else {
        INFO("ndim =  %g",ndim);}
    ...
    for (vector<classGPs *>::iterator ite = boundaryGPs.begin(); ite != boundaryGPs.end(); ++ite) {
        classGPs *tempGP = *ite;
        vector<double> n0 = elems[tempGP->getCell()]->getn0(nodes, ndim);// This gets the original normal N. 
        classGPs *tempGPF;
        vector<double> Point = tempGP->getXYZ();
        // Here we adjust the vector elements. I'm not sure why. Not included in the original traction equations, taken from the pressure.
        ...
        multTensorVector(FFinvT, ndim, ndim, n0, ndim, n); // n = FinvT * N0  
        multTensorVector(stress, ndim, ndim, n, ndim, traction); // traction = stress * n 
        for (int a = 0; a < neighbours.size(); a++) {
            for (int i = 0; i < ndim; i++) {
                int indi11 = neighbours[a] + nodes.size() * i;
                force[indi11] += weight * std::abs(J) * phi[a] * Jacobian * traction[i];
            }
        }
        //INFO("new normal found, (%g,%g,%g)",Jacobian *n[0],Jacobian *n[1],Jacobian *n[2]);
        //INFO("traction vector, (%g,%g,%g)",Jacobian *traction[0],Jacobian *traction[1],Jacobian *traction[2]);
        delete tempGPF;
        delete tempGP;
    }
}
...
extern void BCsStressInst(const vector<double> &uK, vector<classNodes *> &nodes, vector<classElements *> &elements,
    ...
    if (ndim ==2){
        ...
        stress[2] = stressTensor[1];
        stress[3] = stressTensor[4];
        //INFO("created 2D tensor, (%g,%g,%g,%g)",stress[0],stress[1],stress[2],stress[3]);
    }
    else if (ndim ==3){    //fill in symmetric off-diagonal entries
        stress = stressTensor;
        stress[3] = stressTensor[1];
...
extern void BCsVolHeatFluxGaussian(const vector<double> &uK, vector<classGPs *> &GPs, vector<classNodes *> &nodes, classVolHeatFluxGaussian *&BCsVolHeatFlux, int &ndim,
    ...
    for (int igp = 0; igp < GPs.size(); igp++)
    {
        ...
        GaussPoint->updatePositionWithNodalDisplacement(uK,numNodes);
        vector<double> XYZ = GaussPoint->getXYZCurr();

        //vector<double> XYZ =  GaussPoint->getXYZ(); reference configuration
