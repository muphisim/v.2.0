...
/*! Constructor for Neumann Stress BCs Ramp
  @param[in] type Type of Neumann BCs
  @param[in] listOfElements Nodes at which the forces are applied
  @param[in] Sxx Elements of stress tensor
  @param[in] t0 Init time to apply the pressure
  @param[in] tf Final time to apply the pressure
*/
classStressRamp::classStressRamp(string type, vector<classElements *> &listOfElements, 
...
/*! Constructor for Neumann Stress BCs Inst
  @param[in] type Type of Neumann BCs
  @param[in] listOfElements Nodes at which the forces are applied
  @param[in] Sxx Elements of stress tensor
  @param[in] t0 Init time to apply the pressure
  @param[in] tf Final time to apply the pressure
*/
classStressInst::classStressInst(string type, vector<classElements *> &listOfElements, 
                                     double Sxx, double Sxy, double Sxz,
                                     double Syy, double Syz, double Szz,
                                     double t0, double tf) : classNeumannBCs(type) {
    _Sxx =Sxx;
...
classTractionInst::classTractionInst(string type, vector<classElements *> &listOfElements, 
                                     ...
                                     double t0, double tf,
                                     double Tx0, double Ty0, double Tz0) : classNeumannBCs(type) {
    _Tx = Tx;
    _Ty = Ty;
    _Tz = Tz;
    _listOfElements = listOfElements;
...
classPressureRamp::classPressureRamp(string type, vector<classElements *> &listOfElements, double P, double t0,
                                     double tf, double P0) : classNeumannBCs(type) {
    _P = P;
    _listOfElements = listOfElements;
    _tf = tf;
    _t0 = t0;
