...
TimeStepping::TimeStepping(double tol):
    ...
    _times(1,0),_nsteps(),_forced(false), _stepMaxInterval(){}
...
TimeStepping::TimeStepping(const TimeStepping& src):
    ...
    _forced(src._forced),
    _stepMaxInterval(src._stepMaxInterval)
{
}
...
void TimeStepping::setTimeStep(double dt)
{
    ...
    if (_stepMaxInterval.size() > 0)
    {
      for (int i=0; i<_nsteps.size(); i++)
      {
        double lowerBound = _times[i];
        double upperBound = _times[i+1];
        if ((_lastTime > lowerBound) and (_lastTime < upperBound))
        {
          _timeStep = std::min(_stepMaxInterval[i],_timeStep);
          break;
        }
      } 
    }
    ...
    for (int i=0; i<_nsteps.size(); i++)
    {
        ...
        if (fabs(_lastTime-lowerBound) < _tol*_endTime)
        {
            ...
            if (_stepMaxInterval.size() > 0)
            {
              _timeStep = std::min(_stepMaxInterval[i],(upperBound-lowerBound)/_nsteps[i]);
            }
            else
            {
...
void TimeStepping::setNumberOfSteps(double time, int nstep, double dtMax)
{
    _times.push_back(time);
    _nsteps.push_back(nstep);
    _stepMaxInterval.push_back(dtMax);
}
...
void TimeStepping::initFromOptions()
{
    ...
    if (_numSteps >0)
    {
      ...
      if (_stepMaxInterval.size() > 0)
      {
        _timeStep = std::min(_timeStep,_stepMaxInterval[0]);
      }
    }
    else
    {
