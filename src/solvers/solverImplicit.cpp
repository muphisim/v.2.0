...
void classSolverImplicitBase::calculate(vector<classNeumannBCs *> &NeumannBCs, vector<classGPs *> &GPs, int sim,
    ...
    if (_flagFSI)
    {
      dtFSI = FSIAdapter::getInstance()->initialize();
    ...
    while (_timeSteppingPlan.endCheck()) 
    {
        ...
        {
            if(FSIAdapter::getInstance()->isActionRequired(FSIAdapter::getInstance()->actionWriteIterationCheckpoint()))
            {
                makeACheckPoint(GPs,nlsys);
                FSIAdapter::getInstance()->makeACheckPoint(timeRunFSI,dtFSI);
                FSIAdapter::getInstance()->markActionFulfilled(FSIAdapter::getInstance()->actionWriteIterationCheckpoint());
            };
            FSIAdapter::getInstance()->readBlockVectorData();
            timeRunFSI += dtFSI;
            dtFSISubstepping = 0.;
        }
        ...
        while (true) 
        {
            ...
            if ((!std::isfinite(normRes)) || (!std::isfinite(normResExtra)) || (!std::isfinite(normResStoch))) 
            {
                if (_comRank == 0)
                {
                    ...
                    {
                        WARNING("Residuals are not finite numbers : Mech-absolute error %.16g  Extra-absolute error %.16g Stoch-absolute error %.16g", normRes, normResExtra,normResStoch);
                    }
                    else if (extraDofFlag)
                    {
                        WARNING("Residuals are not finite numbers : Mech-absolute error %.16g  Extra-absolute error %.16g", normRes, normResExtra);
                    }
                    else if (StochasticFlag)
                    {
                        WARNING("Residuals are not finite numbers : Mech-absolute error %.16g Stoch-absolute error %.16g", normRes,normResStoch);
                    }
                    else
                    {
                        WARNING("Residuals are not finite numbers : Mech-absolute error %.16g", normRes);
                    }
                }
                flagConver = false;
            ...
            else
            {
                ...
                if (_comRank == 0) {
                    if (extraDofFlag and StochasticFlag) 
                    {
                        INFO("Iteration %i: Mech-absolute error %e Mech-relative error %e  Extra-absolute error %e Extra-relative error %e Stoch-absolute error %e Stoch-relative error %e", 
                                    ite2, normRes, tolRes, normResExtra,tolExtra ,normResStoch,tolStoch);
                    }
                    else if (extraDofFlag)
                    {
                        INFO("Iteration %i: Mech-absolute error %e Mech-relative error %e  Extra-absolute error %e Extra-relative error %e", 
                                    ite2, normRes, tolRes, normResExtra,tolExtra);
                    }
                    else if (StochasticFlag)
                    {
                        INFO("Iteration %i: Mech-absolute error %e Mech-relative error %e Stoch-absolute error %e Stoch-relative error %e", 
                                    ite2, normRes, tolRes, normResStoch,tolStoch);
                    }
                    else
                    {
                        INFO("Iteration %i: Mech-absolute error %e Mech-relative error %e", 
                                    ite2, normRes, tolRes);
                    }
                }
                ...
                if ((normRes < _absTol) && (normResExtra < _absTol) && (normResStoch < _absTol))
                {
                    if (_comRank == 0)
                    {
                        if (extraDofFlag and StochasticFlag) 
                        {
                            INFO("Converged Iteration %i on absolute residual: Mech-absolute error %.16g  Extra-absolute error %.16g Stoch-absolute error %.16g", ite2, normRes, normResExtra,normResStoch);
                        }
                        else if (extraDofFlag)
                        {
                            INFO("Converged Iteration %i on absolute residual: Mech-absolute error %.16g  Extra-absolute error %.16g", ite2, normRes, normResExtra);
                        }
                        else if (StochasticFlag)
                        {
                            INFO("Converged Iteration %i on absolute residual: Mech-absolute error %.16g Stoch-absolute error %.16g", ite2, normRes,normResStoch);
                        }
                        else
                        {
                            INFO("Converged Iteration %i on absolute residual: Mech-absolute error %.16g", ite2, normRes);
                        }
        ...
        if (_flagFSI && FSIstepDone)
        {
            FSIAdapter::getInstance()->updateDisplacementInterface(dtFSI,uK,_numNodes);
            dtFSI = FSIAdapter::getInstance()->advance(dtFSI);
            if(FSIAdapter::getInstance()->isActionRequired(FSIAdapter::getInstance()->actionReadIterationCheckpoint()))
            {
                restoreACheckPoint(GPs,nlsys);
                FSIAdapter::getInstance()->restoreACheckPoint(timeRunFSI,dtFSI);
                FSIAdapter::getInstance()->markActionFulfilled(FSIAdapter::getInstance()->actionReadIterationCheckpoint());
        ...
        if (_flagFSI && FSIstepDone)
        {
            if (!FSIAdapter::getInstance()->isCouplingOngoing())
            {
    ...
    if (_flagFSI)
    {
        FSIAdapter::getInstance()->finalize();
    }
#endif
    delete nlsys;
};
                            ...
                            double dt, map<pair<int,int>,double>& stiffBC, bool tangentEstimation, double FSIstepRatio)
{
    ...
    if (_flagFSI)
    {
        FSIAdapter::getInstance()->updateForceInterface(nod, elem, ndim, timeRun, dt, allField,extForce,_numNodes, FSIstepRatio, tangentEstimation, stiffBC);
    }
#endif      
    nlsys->zeroRHS();
