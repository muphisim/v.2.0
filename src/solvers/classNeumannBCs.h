...
/*! \brief  This class defines each Neumann node in the domain*/
class classStressRamp : public classNeumannBCs {

private:
    vector<classElements *> _listOfElements; // They will be ndim-1 dimension
    //double _Tx, _Ty, _Tz; // traction following 3 component
    double _Sxx, _Sxy, _Sxz, _Syy, _Syz, _Szz; // symmetric stress tensor with 6 components
    double _t0, _tf; //
public:
    classStressRamp(string type, vector<classElements *> &listOfElements, double Sxx, double Sxy, double Sxz,double Syy, double Syz, double Szz, double t0, double tf);
    inline vector<classElements *>& getListOfElements() { return _listOfElements; }; // Common
    inline void getTraction(double timeRun, double dt, double& Sxx, double& Sxy, double& Sxz,double& Syy, double& Syz, double& Szz) 
    {
        Sxx =0;
        Sxy =0;
        Sxz =0;
        ...
        {
            Sxx =_Sxx/ (_tf - _t0) * (timeRun - _t0);
            Sxy =_Sxy/ (_tf - _t0) * (timeRun - _t0);
            Sxz =_Sxz/ (_tf - _t0) * (timeRun - _t0);
            Syy =_Syy/ (_tf - _t0) * (timeRun - _t0);
            Syz =_Syz/ (_tf - _t0) * (timeRun - _t0);
            Szz =_Szz/ (_tf - _t0) * (timeRun - _t0);
        } 
    }; // Common
    virtual ~classStressRamp() {
...
/*! \brief  This class defines each Neumann node in the domain*/
class classStressInst : public classNeumannBCs {
...
private:
    ...
    double _t0, _tf; //
public:
    classStressInst(string type, vector<classElements *> &listOfElements, double Sxx, double Sxy, double Sxz,double Syy, double Syz, double Szz, double t0, double tf);
    inline vector<classElements *>& getListOfElements() { return _listOfElements; }; // Common
    inline void getTraction(double timeRun, double dt, double& Sxx, double& Sxy, double& Sxz,double& Syy, double& Syz, double& Szz) 
    {
        Sxx =0;
        ...
        if (timeRun >= _t0 && timeRun <= _tf)
        {
            ...
            Syz =_Syz;
            Szz =_Szz;
            //INFO("read in inst stress, (%g,%g,%g,%g,%g,%g)",Sxx,Sxy,Sxz,Syy,Syz,Szz);
        } 
    }; // Common
    virtual ~classStressInst() {
...
private:
    vector<classElements *> _listOfElements; // They will be ndim-1 dimension
    double _Tx, _Ty, _Tz; // traction following 3 component
    double _Tx0, _Ty0, _Tz0;
    double _t0, _tf; //
public:
    classTractionInst(string type, vector<classElements *> &listOfElements, double Tx, double Ty, double Tz, double t0, double tf, double Tx0=0, double Ty0=0, double Tz0=0 );
    inline vector<classElements *>& getListOfElements() { return _listOfElements; }; // Common
...
private:
    ...
    double _t0, _tf; //
public:
    classTractionRamp(string type, vector<classElements *> &listOfElements, double Tx, double Ty, double Tz, double t0, double tf, double Tx0=0, double Ty0=0, double Tz0=0 );
    inline vector<classElements *>& getListOfElements() { return _listOfElements; }; // Common
    inline void getTraction(double timeRun, double dt, double& Tx, double& Ty, double& Tz) 
    {
        Tx =0;
        ...
        if (timeRun >= _t0 && timeRun <= _tf)
        {
            Tx = _Tx0 + _Tx / (_tf - _t0) * (timeRun - _t0);
            Ty = _Ty0 + _Ty / (_tf - _t0) * (timeRun - _t0);
            Tz = _Tz0 + _Tz / (_tf - _t0) * (timeRun - _t0);
        } 
    }; // Common
...
private:
    vector<classElements *> _listOfElements; // They will be ndim-1 dimension
    double _P0, _P, _t0, _tf;
public:
    classPressureRamp(string type, vector<classElements *> &listOfElements, double P, double t0, double tf, double P0=0);

    inline vector<classElements *>& getListOfElements() { return _listOfElements; }; // Common
    inline double getPressure(double timeRun, double dt) {
        double P = 0;
        if (timeRun >= _t0 && timeRun <= _tf) {
            P = _P0+_P / (_tf - _t0) * (timeRun - _t0);
        } else {
            P = 0.;
        }
        return P;
