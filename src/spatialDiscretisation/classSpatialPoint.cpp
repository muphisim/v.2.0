...
/*! \brief This function update the current location with displacement unknow
  @param[in] uK displacement vector
  @param[in] numNodes number of nodes
 */
void classSpatialPoint::updatePositionWithNodalDisplacement(const vector<double> &uK, int numNodes)
{
  this->xyzCurr = this->xyz;
  int numNeighbours = neighbours.size();
  for (int a = 0; a < numNeighbours; a++)
  {
      int ind = neighbours[a];
      for (int i = 0; i < ndim; i++) 
      {
          this->xyzCurr[i] += uK[ind + i * numNodes]*phi[a];
      }
  }
};
