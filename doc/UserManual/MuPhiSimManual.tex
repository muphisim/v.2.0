...
\end{figure}
\item Poroelasticity, keyword \textbf{*Linear-Poroelastic}. Next line, Young's modulus, Poisson's ratio and the Biot-Willis coefficient $\alpha$ are defined. 2D plane stress/ plane strain can be defined by the keyword:  \textit{PlaneStress}/ \textit{PlaneStrain}) after two material parameters as in the linear elastic model. The remaining poroelastic parameters are defined in ExtraDof using the keyword \textbf{*DarcyLaw}, after which the permeability, viscosity, unconstrained specific storage and Biot-Willis coefficient are defined. The following file is taken as an example:
\begin{lstlisting}
*Linear-Poroelastic
21,0.35,1,PlaneStrain
*ExtraDof
*DarcyLaw
1e-06,2.67e-05,33.3,1
\end{lstlisting}
More detail on the poroelasticity constitutive model can be found in Chapter 6.
\end{enumerate}
...
%and deformations are taken to be small.
...
% \begin{equation}   
% \theta = \frac{1}{3H} \left( \sigma_{x} + \sigma_{y} + \sigma_{z} \right) + \frac{p}{R}
% \label{eqn:volumefrac1}
% \end{equation}
%
...
%This can be rewritten analogously with the uncoupled heat transfer analysis equation already implemented in MuPhiSim as: 
...
%as the boundary between the arterial mesh and the porous capillary mesh is artificial, and solutes can flow freely with the blood. 
...
%\bar{q}_S=L_p(p - P_f) %, \: \forall \bm{X} \in \partial \Omega_{n}^{FSI} \\
\end{cases}
\label{eqn:starling_bc}
\end{equation}
%where $L_p$, $P_f$, and $p$ are defined as before. 
where $\bar{q}_S$ is defined in Equation \eqref{eqn:starling_new}.
%It is noted that, as $\bm{N}$ is the outward normal to the surface, the sign is positive.
...
\end{equation}
The weak form of the second governing equation is derived by multiplying Equation (\ref{eqn:flux2}) by the scalar virtual test function $\delta p$ and integrating over the reference volume $\Omega_{o}$: %For analogy with the heat transfer analysis already implemented in MuPhiSim we let $C = \frac{1}{Q}$.  
\begin{equation} 
...
%Thus the only term that must be independently implemented is the additional pore pressure coupling term $\alpha  \left(\nabla \cdot \bm{\dot{u}}\right)$. 
...
\begin{tabular}{ p{2cm} p{2.5cm} p{2.5cm} p{2cm} p{2.5cm} p{2.5cm} }
 \toprule
    ...
    $E$ & $21.0$ & kg cm$^{-1}$ s$^{-2}$ & & & \\%$P_f$ & $0.1$ & kg cm$^{-1}$ s$^{-2}$ \\
    
    
    %\hline

    %\hline
    %$G$ & $7.78$ & kg cm$^{-1}$ s$^{-2}$ & $L_p$ & $1.326 \times 10^{-8}$ & kg cm$^{-1}$ s$^{-3}$\\
    %\hline
    % $P_f$ & $0.1$ & kg cm$^{-1}$ s$^{-2}$ \\
    % $L_p$ & $1.326 \times 10^{-8}$ & kg cm$^{-1}$ s$^{-3}$\\
    % %\hline
    % $\rho_s$ & $9.77 \times 10^{-4}$ & kg cm$^{-3}$\\
    % %\hline
    % $\rho_f$ & $1 \times 10^{-3}$ & kg cm$^{-3}$\\
    % $\mu_f$ & $4 \times 10^{-5}$ & kg s$^{-1}$ cm$^{-1}$ \\
    % $\alpha$ & $0, 1$ & \\
    % $\nu$ & $0.35$ & \\
    %\hline
    %\hline
...
\centering
\subfigure[$t=10 $ s]{
\includegraphics[width=0.7\linewidth, trim={4cm 2cm 4cm 2cm}, clip]{imgs/poro-pressure-10s.png}
}
\subfigure[$t=25 $ s]{
\includegraphics[width=0.7\linewidth, trim={4cm 2cm 4cm 2cm}, clip]{imgs/poro-pressure-25s.png}
}
\subfigure[$t=50 $ s]{
\includegraphics[width=0.7\linewidth, trim={4cm 2cm 4cm 2cm}, clip]{imgs/poro-pressure-50s.png}
}
\subfigure[$t=100 $ s]{
\includegraphics[width=0.7\linewidth, trim={4cm 2cm 4cm 2cm}, clip]{imgs/poro-pressure-100s.png}
}
...
With the scale bar for the pore pressure in Figure \ref{fig:FSI_full_p} rescaling at every time step, we plot the pore pressure evolution with $x$ along the centreline $y=0$ at five time points in Figure \ref{fig:FSI_pore_pressure} to consider this quantity in more detail, which shows the pore pressure increasing steadily with time as we expect. %It is noted that for the earlier time steps, the range of pore pressure values is higher, yet by $50 $ s the pore pressure has converged to a steady value for all $x$. This is likely due to the fact that the fluid flow is fully developed at around $50 $ s so less fluctuation is seen. 
\begin{figure}[htbp]
\centering
